...
import MPOStatementReport from "../MPOStatementReport/MPOStatementReport";
import MPOStatementReportExcel from "../MPOStatementReportExcel/MPOStatementReportExcel";
...
const MPOStatement = () => {
    ...
    const [reportType, setReportType] = useState('MPO Statement');
    ...
    const clearFilters = () => {
        ...
        setAreaManager('');
        setCustomer('');
        setReportType('MPO Statement');
    };

    const handlePrint = MPOStatementReport({
        reportType,
        filteredOrders,
        orderReturns,
        firstDate,
    ...
    const handleDownloadExcel = MPOStatementReportExcel({
        reportType,
        filteredOrders,
        orderReturns,
        firstDate,
    ...
    return (
        <>
            <div>
                <PageTitle from={"Accounts"} to={"MPO statement"} />
            </div>
            <div className="bg-white pb-1">
                <div>
                    <h1 className="px-6 py-3 font-bold">MPO statement report</h1>
                                ...
                                <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4 my-6 px-6">
                                    ...
                                    <div className='col-span-1 md:col-span-2'>
                                        ...
                                        <Select
                                            ...
                                            onChange={(e) => setReportType(e?.value || 'MPO Statement')}
                                            options={[
                                                { value: "MPO Statement", label: "MPO Statement" },
                                                ...(orderedBy ? [{ value: "MPO wise MPO Statement", label: "MPO wise MPO Statement" }] : []),
                                                ...(territory ? [{ value: "Territory wise MPO Statement", label: "Territory wise MPO Statement" }] : []),
                                                ...(areaManager ? [{ value: "Area Manager wise MPO Statement", label: "Area Manager wise MPO Statement" }] : []),
                                                ...(customer ? [{ value: "Customer wise MPO Statement", label: "Customer wise MPO Statement" }] : [])
                                            ]}
                                            isClearable
                                            isSearchable
                                        />

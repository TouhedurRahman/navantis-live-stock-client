...
import SalesReturnsReport from "../SalesReturnsReport/SalesReturnsReport";
...
const SalesReturnsList = () => {
    ...
    const groupedReturns = returns.flatMap((returnItem) =>
        ...
        const key = `${product.productName}-${product.netWeight}-${product.batch}-${product.expire}-${product.tradePrice}-${product.actualPrice}-${product.date}`;

        if (acc[key]) {
            acc[key].totalQuantity += product.totalQuantity;
        } else {
            acc[key] = { ...product };
        }
    ...
    const products = Object.values(groupedReturns);
    ...
    const handlePrintWithAP = SalesReturnsReport({ invoiceWithAP, firstDate, lastDate, totalUniqueProducts, totalUnit, totalTP, totalAP, filteredProducts });

    const handlePrint = SalesReturnsReport({ firstDate, lastDate, totalUniqueProducts, totalUnit, totalTP, totalAP, filteredProducts });
    ...
    return (
        <div>
            ...
            <div className="bg-white pb-1">
                <div>
                    <h1 className="px-6 py-3 font-bold">Depot sales returns products list</h1>
